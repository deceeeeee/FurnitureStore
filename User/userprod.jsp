		...
		<div id="content">
			<div id="products">
					...
					while(result.next()){
						%>
							...
							<div class="prodinfo">
								...
								<a href="cart_handle.jsp?product_id=<%=result.getString("product_id")%>&product_name=<%=result.getString("product_name")%>&color=<%=result.getString("color")%>&size=<%=result.getString("size")%>&image=<%=result.getString("image")%>&price=<%=result.getString("price")%>">+ Add to cart</a>
							</div>
				<%
				...
				<%
					...
					String fetch = "SELECT KEYWORDS FROM KEYWORDS WHERE USERNAME = '"+user+"' ORDER BY TIMESTAMP DESC ";
					...
					while(i < 10){
						...
						if(result.next()){
							...
							keywords[i] = result.getString("keywords");	
					...
					container = "OR PRODUCT_ID LIKE (SELECT CODE FROM FURNITURE_CATEGORY WHERE CATEGORY LIKE '"+keywords[j]+"') OR PRODUCT_NAME LIKE '%"+keywords[j]+"%' OR MATERIAL LIKE '%"+keywords[j]+"%' OR COLOR LIKE '%"+keywords[j]+"%' OR PRODUCER LIKE '%"+keywords[j]+"%' ";
					recommend += container;
					
					}
					
					recommend = recommend + "ORDER BY CLICKED DESC";
